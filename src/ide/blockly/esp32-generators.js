...
generator.forBlock['gpio_pwm_write'] = function(block) {
    ...
    if (!generator.definitions_[pwmVar]) {
        generator.definitions_[pwmVar] = `${pwmVar} = PWM(Pin(${pin}), freq=5000, duty=0)`;
    }
...
generator.forBlock['sensor_analog_percent'] = function(block) {
    ...
    return [`round((${code} / 4095) * 100)`, generator.ORDER_FUNCTION_CALL];
};

generator.forBlock['sensor_ldr_light'] = generator.forBlock['gpio_analog_read'];
generator.forBlock['sensor_soil_moisture'] = generator.forBlock['gpio_analog_read'];
generator.forBlock['sensor_potentiometer'] = generator.forBlock['gpio_analog_read'];
...
generator.forBlock['sensor_limit_switch'] = generator.forBlock['sensor_ir_obstacle'];
...
generator.forBlock['actuator_onboard_led'] = function(block) {
    ensureMachineImport();
    const state = block.getFieldValue('STATE');
    generator.definitions_['onboard_led'] = `onboard_led = Pin(2, Pin.OUT)`;
...
generator.forBlock['actuator_buzzer_tone'] = function(block) {
    ensureMachineImport();
    const pin = block.getFieldValue('PIN');
    const freq = generator.valueToCode(block, 'FREQ', generator.ORDER_ATOMIC) || '500';
    const duration = generator.valueToCode(block, 'DURATION', generator.ORDER_ATOMIC) || '100';
    const pwmVar = `buzzer_pwm_${pin}`;
    if (!generator.definitions_[pwmVar]) {
        generator.definitions_[pwmVar] = `${pwmVar} = PWM(Pin(${pin}), duty=0)`;
    }
...
generator.forBlock['actuator_buzzer_note'] = function(block) {
    ...
    const toneBlock = {
        ...
        valueToCode: (name, order) => {
            ...
            if (name === 'DURATION') return generator.valueToCode(block, 'DURATION', order);
            return null;
        }
    };
    return generator.forBlock['actuator_buzzer_tone'](toneBlock);
};

generator.forBlock['actuator_servo_positional'] = function(block) {
        ensureMachineImport();
        const pin = block.getFieldValue('PIN');
        // --- MODIFIED: Use getFieldValue for the angle field ---
        const angle = block.getFieldValue('ANGLE') || '90';
        const servoVar = `servo_${pin}`;
        
        // This logic remains the same
        if (generator.isLiveMode) {
            const code = `
...
generator.forBlock['ble_setup'] = function(block) {
    ...
    const name = generator.valueToCode(block, 'NAME', 0) || '"ESP32"';
